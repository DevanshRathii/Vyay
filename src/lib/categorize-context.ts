import type { Category, MerchantRule } from "@/lib/db/schema";

/**
 * Pure categorization logic — deliberately has NO `db` import, so this
 * whole module is safe to bundle client-side (src/lib/parser-sync.ts, for
 * keyed accounts whose data the server can't decrypt to re-derive
 * categories/merchant names for). `categorize.ts` re-exports everything
 * here for backward compatibility and adds the DB-touching wrappers
 * (loadCategorizerContext, ensureDefaultCategories, findCategoryByName).
 */
...
export function buildCategorizerContext(categories: Category[], userRules: MerchantRule[]): CategorizerContext {
  return {
    userRules,
    categoriesById: new Map(categories.map((c) => [c.id, c])),
    categoryIdByLowerName: new Map(categories.map((c) => [c.name.toLowerCase(), c.id])),
  };
}
 ...
 * clean display name. Checked at ingest/reparse time (see ingest.ts and
 * merchant.ts) against `merchantNormalized`, overriding a mangled or
 * corporate-suffixed extraction ("Uber India Systems" → "Uber") with a
 * confidence bump to 0.95. Not used by categorize() itself — this is purely
 * a display/confidence concern.
 */
